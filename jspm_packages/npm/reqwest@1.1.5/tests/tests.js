...
(function(process) {
  (function(ajax) {
    ...
    sink('Setup', function(test, ok, before, after) {
      before(function() {
        ajax.ajaxSetup({dataFilter: function(resp, type) {
      ...
      test('JSONP', function(complete) {
        ...
        ajax({
          ...
          success: function(resp) {
            ...
            ok(resp && resp.boosh == 'boosh', 'correctly evaled response for unique generated cb as JSONP');
            complete();
          }
        });
      });
        ...
        ajax({
          ...
          success: function(resp) {
            if (resp == null) {
              ...
              ok(resp && resp.documentElement && resp.documentElement.firstChild && (/error/i).test(resp.documentElement.firstChild.nodeValue), 'XML response reports parsing error');
              complete();
            }
          },
          error: function() {
    ...
    sink('JSONP', function(test, ok) {
      ...
      test('Unnamed callback in query string', function(complete) {
        ajax({
          ...
          success: function(resp) {
            ...
            ok(resp && resp.boosh == 'boosh', 'correctly evaled response as JSONP with custom wildcard cb');
            complete();
          }
        });
      });
      test('No callback, no query string', function(complete) {
        ajax({
          ...
          success: function(resp) {
            ok(resp, 'received response for custom wildcard callback');
            ok(resp && resp.boosh == 'boosh', 'correctly evaled response as JSONP with custom cb not in url');
            complete();
          }
        });
      });
      test('No callback in existing query string', function(complete) {
        ajax({
          ...
          success: function(resp) {
            ...
            ok(resp && resp.query && resp.query.somevar == 'some long str here', 'correctly evaluated response as JSONP with echo callback');
            complete();
          }
        });
      });
      test('Append data to existing query string', function(complete) {
        ajax({
          ...
          type: 'jsonp',
          data: {
            somevar: 'some long str here',
            anothervar: 'yo ho ho!'
          },
          success: function(resp) {
            ok(resp && resp.query, 'received response from echo callback');
            ok(resp && resp.query && resp.query.somevar == 'some long str here', 'correctly sent and received data object from JSONP echo (1)');
      ...
      test('Generate complete query string from data', function(complete) {
        ajax({
          ...
          success: function(resp) {
            ok(resp && resp.query, 'received response from echo callback');
            ok(resp && resp.query && resp.query.somevar == 'some long str here', 'correctly sent and received data array from JSONP echo (1)');
    ...
    if (window.XMLHttpRequest && ('withCredentials' in new window.XMLHttpRequest())) {
      sink('Cross-origin Resource Sharing', function(test, ok) {
        ...
        test('get cookie from other origin', 1, function() {
          ajax({
            url: 'http://' + window.location.hostname + ':5678/get-cookie-value',
            type: 'text',
            method: 'get',
            crossOrigin: true,
            withCredentials: true,
    ...
    sink('Connection Object', function(test, ok) {
      ...
      function testXhrGetUrlAdjustment(url, data, expectedUrl, complete) {
        ...
        try {
          ...
          ok(FakeXHR.last.methodCallArgs('open', 0, 1) == expectedUrl, 'second arg of open() is URL with query string');
      ...
      test('ajax() appends GET URL with ?`data` (serialized object)', function(complete) {
        testXhrGetUrlAdjustment('/tests/fixtures/fixtures.html', {
          bar: 'foo',
          thunk: 'baz'
        }, '/tests/fixtures/fixtures.html?bar=foo&thunk=baz', complete);
      });
      test('ajax() appends GET URL with &`data` (serialized array)', function(complete) {
        testXhrGetUrlAdjustment('/tests/fixtures/fixtures.html?x=y', [{
          ...
          name: 'thunk',
          value: 'baz'
        }], '/tests/fixtures/fixtures.html?x=y&bar=foo&thunk=baz', complete);
      });
    });
    ...
    sink('Ender Integration', function(test, ok) {
      ...
      test('$.toQueryString alias for reqwest.toQueryString, not bound to boosh', function(complete) {
    ...
    sink('Promises', function(test, ok) {
      test('always callback is called', function(complete) {
        ajax({url: '/tests/fixtures/fixtures.js'}).always(function() {
          ok(true, 'called complete');
          complete();
        });
      });
      ...
      test('success does not chain with then', 2, function() {
        ajax({
          ...
          ok(resp && resp !== 'some independent value', 'then callback fired');
        });
      });
      test('then & always handlers can be added after a response is received', 2, function() {
        var a = ajax({
          url: '/tests/fixtures/fixtures.json',
          type: 'json'
        }).always(function() {
      ...
      test('then is chainable after a response is received', 2, function() {
        var a = ajax({
          url: '/tests/fixtures/fixtures.json',
          type: 'json'
        }).always(function() {
      ...
      test('failure handlers can be added after a response is received', function(complete) {
        var a = ajax({
          url: '/tests/fixtures/invalidJSON.json',
          type: 'json'
        }).always(function() {
      ...
      test('xmlHttpRequest', function(complete) {
        ...
        ajax({
          ...
          error: function(err, msg) {
            ...
            ok(tt > 200 && tt < 300, 'timeout close enough to 250 (' + tt + ')');
            complete();
          }
        });
      });
